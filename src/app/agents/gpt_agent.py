"""GPT Agent - OpenAI GPT model via Azure OpenAI Service."""
...
class GPTAgent(IAgent):
    """
    GPT model identity agent (OpenAI via Azure).
    ...
    def __init__(self, deployment_name: str, config: ModelConfig) -> None:
        """Initialize the GPT agent with Azure OpenAI client."""
        ...
        timeout = httpx.Timeout(settings.get_agent_timeout("gpt"), connect=settings.agent_connect_timeout)
    ...
    async def respond(self, message: str) -> str:
        """Generate response from GPT via Azure OpenAI."""
    ...
    def create_af_agent(self) -> object:
        """Create Agent Framework runtime agent for GPT."""
    ...
    async def run_turn(self, history: list[dict]) -> ModelResponse:
        ...
        with tracer.start_as_current_span("agent.gpt.run_turn") as span:
            span.set_attribute("agent.name", self.name)
            try:
                prompt = prompt_loader.render("conversation.jinja2", history=history)
                af_agent = self.create_af_agent()
                raw_response = await asyncio.wait_for(af_agent.run(prompt), timeout=settings.get_agent_timeout("gpt"))
