"""Grok Agent - xAI Grok model via Azure OpenAI Service."""
...
class GrokAgent(IAgent):
    """
    Grok model identity agent (xAI via Azure).
    
    Connects to Azure OpenAI Service using Azure AD authentication.
    ...
    def __init__(self, deployment_name: str, config: ModelConfig) -> None:
        """Initialize the Grok agent with Azure OpenAI client."""
        ...
        timeout = httpx.Timeout(settings.get_agent_timeout("grok"), connect=settings.agent_connect_timeout)
    ...
    async def respond(self, message: str) -> str:
        """Generate response from Grok via Azure OpenAI."""
        ...
        with tracer.start_as_current_span(f"agent.{self.name.lower()}.respond") as span:
            ...
            try:
                ...
                response = await asyncio.wait_for(
                    ...
                    timeout=settings.get_agent_timeout("grok")
                )
    ...
    def create_af_agent(self) -> object:
        """Create Agent Framework runtime agent for Grok."""
        ...
        return Agent(
            name="grok",
            instructions=prompt_loader.render("system-prompt.jinja2", model_name="grok"),
            client=client,
        )
    ...
    async def run_turn(self, history: list[dict]) -> ModelResponse:
        ...
        with tracer.start_as_current_span("agent.grok.run_turn") as span:
            span.set_attribute("agent.name", self.name)
            try:
                prompt = prompt_loader.render("conversation.jinja2", history=history)
                af_agent = self.create_af_agent()
                raw_response = await asyncio.wait_for(af_agent.run(prompt), timeout=settings.get_agent_timeout("grok"))
